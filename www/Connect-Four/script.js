...
const game = new Vue({
    ...
    dropPiece(e) {
      ...
      for (let i = this.grid.length - 1; i >= 0; i--) {
        if (!this.grid[i][column].color) {
          this.grid[i][column].color = this.currentTurnColorName;
          this.refreshGrid();
  ...
  computed: {
    currentTurnColor() {
      return this.redTurn ? "紅" : "黃";
    },

    currentTurnColorName() {
    ...
    message() {
      if (this.isADraw) {
        ...
        return `<b class="${this.currentTurnColorName}">${this.currentTurnColor} WINS! 🎉</b>`;
      } else {
        return `<b class="${this.currentTurnColorName}">${this.currentTurnColor}</b> 的回合`;
      }
    },
